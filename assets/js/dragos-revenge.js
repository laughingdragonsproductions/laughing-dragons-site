(() => {
  ...
  const MODES = {
    easy: { id: "easy", moveInterval: 0.75, timeBonus: 30, label: "Easy", zen: false },
    hard: { id: "hard", moveInterval: 0.32, timeBonus: 0, label: "Hard", zen: false },
    zen: { id: "zen", moveInterval: 0.75, timeBonus: 0, label: "Zen", zen: true },
  };

  function isZenMode() {
    return !!(MODES[difficulty] || MODES.hard).zen;
  }
  ...
  function startLevel(idx) {
    ...
    parseLevel(def);
    const mode = MODES[difficulty] || MODES.hard;
    timeLeft = mode.zen ? 0 : def.timeLimit + mode.timeBonus;
    updateHud();
    draw();
  ...
  function updateHud() {
    ...
    if (els.timer) {
      if (isZenMode()) {
        ...
        els.timer.classList.add("is-zen");
        els.timer.setAttribute("aria-label", "No time limit");
      } else {
        els.timer.classList.remove("is-zen");
        els.timer.removeAttribute("aria-label");
  ...
  function tryExecutePush(blocks, dx, dy) {
    ...
    } else if (!crushPickupAt(tailX, tailY)) {
  ...
  function bindUi() {
    ...
    const diffZen = $("dr-diff-zen");
    if (difficulty === "easy" && diffEasy) diffEasy.checked = true;
    if (difficulty === "hard" && diffHard) diffHard.checked = true;
    if (difficulty === "zen" && diffZen) diffZen.checked = true;

    $("dr-btn-play")?.addEventListener("click", () => {
      if (diffZen?.checked) difficulty = "zen";
      else if (diffHard?.checked) difficulty = "hard";
