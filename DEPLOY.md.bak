...
Root `ads.txt` is set for `pub-7048606415692002`. The AdSense publisher script loads on all pages **except** `/kids/**` (Kids Show) and `coming-soon` WIP paths. **Ad units** render on all finished `/games/` and live `/tools/` pages after you paste slot IDs into `config.js`. The homepage includes the script in `<head>` for Google site verification.
